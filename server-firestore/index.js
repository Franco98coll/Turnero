...
// Necesita GOOGLE_APPLICATION_CREDENTIALS o variables de servicio en Render
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: u.id, role: u.role, email: u.email, name: u.name }, JWT_SECRET, { expiresIn: "8h" });
...
app.post("/api/slots", auth("admin"), async (req, res) => {
  ...
  if (!start_time || !end_time || !capacity) return res.status(400).json({ error: "Datos incompletos" });
...
app.get("/api/bookings", auth(), async (req, res) => {
  ...
  if (!isAdmin) q = q.where("user_id", "==", req.user.id).where("status", "!=", "canceled");
  const snap = await q.get();
  const bookings = await Promise.all(
    snap.docs.map(async (d) => {
      const b = { id: d.id, ...d.data() };
      const s = await db.collection(SLOTS).doc(b.slot_id).get();
      return { ...b, start_time: s.data().start_time, end_time: s.data().end_time };
    })
  );
  const now = new Date();
  res.json(isAdmin ? bookings : bookings.filter((b) => b.start_time.toDate() >= now));
});
app.post("/api/bookings", auth(), async (req, res) => {
  ...
  const used = (
    await db.collection(BOOKINGS).where("slot_id", "==", slot_id).where("status", "==", "confirmed").get()
  ).size;
